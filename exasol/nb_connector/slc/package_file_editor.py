...
def ensure_pip_tools_in_package_file(file_path: Path, build_step_name: str) -> None:
    session = PackageFileSession(file_path)
    all_phases = [
        phase for build_step in session.package_file_config.build_steps for phase in build_step.phases
    ]
    pip_tools = [phase.tools.pip for phase in all_phases if phase.tools is not None and phase.tools.pip is not None]
    if not pip_tools:
        pip_setup_phase = Phase(name="setup_pip", tools=Tools(pip=Pip(version="21.3.1", needs_break_system_packages=False, comment="Auto-generated by notebook connector")))
