...
from unittest.mock import MagicMock, create_autospec, PropertyMock, call
...
from exasol.current_container_finder import CurrentContainerFinder, IPRetriever
...
def create_docker_client_mock(containers: List[Dict[str, str]]) -> Union[docker.DockerClient, MagicMock]:
    docker_client_mock: Union[MagicMock, docker.DockerClient] = create_autospec(docker.DockerClient)
    container_mocks = [
        create_container_mock(networks) for networks in containers
    ]
...
def create_container_mock(networks: Dict[str, str]) -> Union[MagicMock, Container]:
    ...
    type(container_mock).attrs = PropertyMock(return_value={
        "NetworkSettings": {
            "Networks": {
                name: {
                    "IPAddress": ip
...
class TestSetup:
    ...
    def __init__(self, containers: List[Dict[str, str]], ip_addresses: List[str]):
        ...
        self.current_container_finder = CurrentContainerFinder(docker_client=self.docker_client_mock,
                                                               ip_retriever=self.ip_retriever_mock)
...
def test_ips_in_ips_of_single_container():
    ...
    assert result == mock_cast(test_setup.docker_client_mock.containers.list).return_value[0]
...
def test_ips_in_ips_of_multiple_container():
    ...
    with pytest.raises(RuntimeError, match="Multiple potential current containers found: "):
