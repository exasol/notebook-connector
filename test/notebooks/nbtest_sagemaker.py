...
def _store_aws_credentials(
    store_path: Path, store_password: str, bucket_name: str, role_name: str
):
...
def _create_sagemaker_role_with_policy() -> str:
    ...
    _attach_policy_to_role(iam_client, policy_arn=policy_arn, role_name=role_name)
...
def _remove_sagemaker_endpoint_and_config(endpoint_name: str) -> None:
    ...
    try:
        ...
        if status == "Deleting":
            ...
            if status in ("Creating", "Updating"):
                print(
                    f"Teardown: endpoint '{endpoint_name}' is '{status}', waiting for stable state..."
                )
                try:
...
def test_sagemaker(backend_setup, uploading_hack, notebooks_root):
    ...
    try:
        ...
        os.chdir("../sagemaker")
        run_notebook(
            "sme_init.ipynb", store_file, store_password, hacks=[uploading_hack]
        )
        run_notebook(
            "sme_train_model.ipynb",
            store_file,
            store_password,
            hacks=[get_job_polling_hack()],
        )
        run_notebook(
            "sme_deploy_model.ipynb",
            store_file,
            store_password,
